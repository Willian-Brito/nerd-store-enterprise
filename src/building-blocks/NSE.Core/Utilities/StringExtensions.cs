...
public static class StringExtensions
{
    ...
    {
        var titleEdited = str.Trim().ToLower().RemoveDiacritics();

        titleEdited = UrlizeRegex.Replace(titleEdited, "-");

        if (titleEdited.StartsWith('-'))
            titleEdited = titleEdited[1..];

        if (titleEdited.EndsWith('-'))
            titleEdited = titleEdited[..^1];
    ...
    public static string Urlize(this string str)
    {
        ...
        if (titleEdited.StartsWith("-"))
            titleEdited = titleEdited.Substring(1);

        if (titleEdited.EndsWith("-"))
            titleEdited = titleEdited.Substring(0, titleEdited.Length - 1);
